...
Use `cljs.test/async` with a `done` callback for Promise-based tests. The test
namespace gets compiled into the same JS bundle (same `:sources` in `cljs.edn`).

The specs.html page is generated by scaffold. To run `cljs.test` tests, add a
namespace that calls `cljs.test/run-tests` for the integration test namespaces.
This namespace should be required from a `_spec.cljs` file so scaffold discovers
it, or alternatively invoked via a `goog.debugLoader_` hook after speclj finishes.
The exact integration mechanism will be determined during implementation based on
what scaffold supports.
