...
(describe "Datomic Cloud"
  ...
  (context "schema"
    ...
    (context "spec->attribute"
      ...
      (it "simple string"
        (let [attribute (common-api/spec->attribute :foo :name {:type :string} false)]
      ...
      (it "long"
        (let [attribute (common-api/spec->attribute :foo :names {:type :long} false)]
          (should= :db.type/long (:db/valueType attribute))))

      (it "keyword-ref"
        (let [attribute (common-api/spec->attribute :foo :temper {:type :kw-ref} false)]
          (should= :db.type/ref (:db/valueType attribute))))

      (it "many strings"
        (let [attribute (common-api/spec->attribute :foo :names {:type [:string]} false)]
          (should= :db.type/string (:db/valueType attribute))
          (should= :db.cardinality/many (:db/cardinality attribute))))

      (it "uniqueness"
        (let [attribute (common-api/spec->attribute :foo :name {:type :string :db []} false)]
          (should= nil (:db/unique attribute)))
        (let [attribute (common-api/spec->attribute :foo :name {:type :string :db [:unique-value]} false)]
          (should= :db.unique/value (:db/unique attribute)))
        (let [attribute (common-api/spec->attribute :foo :name {:type :string :db [:unique-identity]} false)]
          (should= :db.unique/identity (:db/unique attribute))))

      (it "component"
        (let [attribute (common-api/spec->attribute :foo :name {:type :ref :db []} false)]
          (should= false (:db/isComponent attribute)))
        (let [attribute (common-api/spec->attribute :foo :name {:type :ref :db [:component]} false)]
          (should= true (:db/isComponent attribute))))

      (it "history"
        (let [attribute (common-api/spec->attribute :foo :name {:type :ref :db []} false)]
          (should= false (:db/noHistory attribute)))
        (let [attribute (common-api/spec->attribute :foo :name {:type :ref :db [:no-history]} false)]
          (should= true (:db/noHistory attribute))))

      (it "fulltext"
        (let [attribute (common-api/spec->attribute :foo :name {:type :string :db []} false)]
          (should= false (:db/fulltext attribute)))
        (let [attribute (common-api/spec->attribute :foo :name {:type :string :db [:fulltext]} false)]
          (should= true (:db/fulltext attribute))))

      (it "indexed"
        (let [attribute (common-api/spec->attribute :foo :names {:type :string} false)]
          (should-not-contain :db/index attribute))
        (let [attribute (common-api/spec->attribute :foo :names {:type :string :db [:index]} false)]
          (should-not-contain :db/index attribute)))
      )
