...
(describe "Migration"
  ...
  (context "with mem db"
    ...
    (helperc/with-schemas [sut/default-migration-schema])
    (with config (assoc db-config :_db db/impl))
    ...
    (it "applied-migrations-names"
      ...
      (sut/create-migration! @config "joe")
      (sut/create-migration! @config "ann")
      (sut/create-migration! @config "LOCK")
      (should= ["ann" "joe"] (sut/applied-migration-names @config)))
    ...
    (context "migrate-unlocked!"

      (it "0 to now"
        (log/capture-logs (sut/migrate-unlocked! @config nil))
        (let [logs (log/parse-captured-logs)]
          (should= ["Running Database Migrations UP"
                    "20230101 - applying migration UP"
                    "20230101 UP"
                    "20230202 - applying migration UP"
                    "20230202 UP"
                    "20230303 - applying migration UP"
                    "20230303 UP"] (map :message logs)))
      ...
      (it "now to 1"
        ...
        (log/capture-logs (sut/migrate-unlocked! @config "20230101"))
        (let [logs (log/parse-captured-logs)]
          (should= ["Running Database Migrations DOWN"
                    "20230303 - applying migration DOWN"
                    "20230303 DOWN"
                    "20230202 - applying migration DOWN"
                    "20230202 DOWN"] (map :message logs)))
